 ...
 *
 * The recap is generated by a single Gemini Flash Lite call -- when the reader *leaves* a book
 * (back arrow, tab close/backgrounding via `pagehide`/`visibilitychange`, switching books; see
 * the leave-triggered effect in App.tsx), or in the background right on reopen when nothing is
 * cached yet for the resumed position (e.g. a leave that predates this cache, or one the leave
 * triggers above never got a chance to run for). Either way, whatever is shown for *this*
 * reopen is always read synchronously from this cache (no network) -- a background reopen call
 * only ever primes it for the *next* one, it never blocks or is waited on. Scoped by user id
 * (falls back to a shared "guest" bucket pre-login), same convention as
 * reading-progress-storage.ts.
 *
